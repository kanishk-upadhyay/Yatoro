...
public class HelpPage: DestroyablePage {
    ...
    private var commandFilter: String?
    ...
    private func renderHelpContent() {
        ...
        var commands = Command.defaultCommands.sorted(by: { $0.name < $1.name })
        var mappings = Mapping.defaultMappings.sorted(by: { $0.key < $1.key })
        
        if let filter = commandFilter {
            commands = commands.filter { $0.name.contains(filter) || ($0.shortName ?? "").contains(filter) }
            mappings = mappings.filter { $0.action.lowercased().contains(filter.lowercased()) }
        }
        
        let maxRows = max(commands.count, mappings.count)
